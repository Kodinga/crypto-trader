    ...
    describe('subscribeToBook()', () => {
        it('should subscribe to symbol', async () => {
            ...
            testScheduler.run(helpers => {
                ...
                const action$ = hotAction('-a', { a: BookActions.subscribeToBook({ symbol: currencyPair }) });
                const state$ = hotState('-');

                const output$ = subscribeToBook(action$, state$, {} as unknown as Dependencies);
    ...
    describe('unsubscribeFromBook()', () => {
        it('should unsubscribe from symbol', async () => {
            ...
            testScheduler.run(helpers => {
                ...
                const action$ = hotAction('-a', { a: BookActions.unsubscribeFromBook({ symbol: currencyPair }) });
                const state$ = hotState('');

                const output$ = unsubscribeFromBook(action$, state$, {} as unknown as Dependencies);
