    ...
    describe('handleSelection()', () => {
        it('clear existing subscriptions before subscribing', async () => {
            ...
            testScheduler.run(helpers => {
                ...
                const action$ = hotAction('---a', { a: SelectionActions.selectCurrencyPair({ currencyPair }) });
                ...
                const output$ = handleSelection(action$, state$, {} as unknown as Dependencies);
    
                expectObservable(output$).toBe('---(abcdef)', {
                    a: CandlesActions.unsubscribeFromCandles({symbol: previousCurrencyPair, timeframe: '1m'}),
                    b: TradesActions.unsubscribeFromTrades({symbol: previousCurrencyPair}),
                    c: BookActions.unsubscribeFromBook({symbol: previousCurrencyPair}),

                    d: CandlesActions.subscribeToCandles({symbol: currencyPair, timeframe: '1m'}),
