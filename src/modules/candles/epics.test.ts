...
import { CandlesActions, SubscribeToCandles, UnsubscribeFromCandles } from './actions';
    ...
    describe('subscribeToCandles()', () => {
        it('should subscribe to symbol', async () => {
            ...
            testScheduler.run(helpers => {
                ...
                const timeframe = '1m';
                const action$ = hotAction('-a', { a: CandlesActions.subscribeToCandles({ symbol: currencyPair, timeframe }) });
                const state$ = hotState('-');

                const output$ = subscribeToCandles(action$, state$, {} as unknown as Dependencies);
    ...
    describe('unsubscribeFromCandles()', () => {
        it('should unsubscribe from symbol', async () => {
            ...
            testScheduler.run(helpers => {
                ...
                const action$ = hotAction('-a', { a: CandlesActions.unsubscribeFromCandles({ symbol: currencyPair, timeframe }) });
                const state$ = hotState('');

                const output$ = unsubscribeFromCandles(action$, state$, {} as unknown as Dependencies);
